...
class SeViLA(Blip2Base):
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        if freeze_vit:
            ...
            logging.info("Vision encoder not frozen")
    ...
    def forward(self, samples,
        ...
        if 'train_loc' in self.task:
            ...
            with torch.no_grad():
                ...
                query_tokens_qa = self.query_tokens_loc.expand(image_embeds_.shape[0], -1, -1)
                query_output_qa = self.Qformer_loc.bert(
                    query_embeds=query_tokens_qa, encoder_hidden_states=image_embeds_,
                    encoder_attention_mask=image_atts_, return_dict=True)
                inputs_t5_qa = self.t5_proj_loc(query_output_qa.last_hidden_state)
        ...
        #     query_tokens_qa = self.query_tokens_loc.expand(select_frames.shape[0], -1, -1)
        #     query_output_qa = self.Qformer_loc.bert(
